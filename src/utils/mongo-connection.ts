...
export default class MongoConnection {
  ...
  private startConnection = () => {
    logger.info(`Connecting to MongoDB at ${this.mongoUrl}`);
    mongoose.connect(this.mongoUrl, this.mongoConnectionOptions)
        .catch(() => {
          logger.error(`Error while Connecting to MongoDB at ${this.mongoUrl}`);
        });
  };
   ...
   */
  private onConnected = () => {
    logger.info(`Connected to MongoDB at ${this.mongoUrl}`);
